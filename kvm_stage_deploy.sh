...
log_file="${script_path}"/log_file_stage.log
. ${script_path}/.env
# kvm_ip (should be sourced from .env file)
# kvm_user (should be sourced from .env file)
# stage_template_vm_name
kvm_remote=" --connect qemu+ssh://$kvm_user@$kvm_ip/system"
...
if [[ -n "$2" ]] 
  then 
    ...
    dirs=($(ssh -o StrictHostKeyChecking=no $kvm_user@$kvm_ip "sudo ls /var/lib/libvirt/images | grep -Ev .qcow2")) && \
    read -p "$(
             f=0
...
# Create VM:
ssh -o StrictHostKeyChecking=no $kvm_user@$kvm_ip "mkdir -p /var/lib/libvirt/images/$project_name"
virt-clone $kvm_remote --original=$stage_template_vm_name --name=$vm_name  --file /var/lib/libvirt/images/$project_name/$vm_name.qcow2
...
# If RAM not set (will be 512MB as in template), set kvm_setmaxmem to 5G:
[[ -z $vm_ram  ]] && \
...
if [[ $vm_disk -gt 2 ]] 
    then
	ssh -o StrictHostKeyChecking=no $kvm_user@$kvm_ip "sudo mv /var/lib/libvirt/images/$project_name/$vm_name.qcow2 /var/lib/libvirt/images/$project_name/$vm_name.qcow2.old && \
...
function wait_for_SSH { 
	ssh -q -o StrictHostKeyChecking=no -i ~/.ssh/${stage_template_vm_ssh_key} ${stage_template_vm_user}@${stage_template_vm_ip} -p ${stage_template_vm_port} exit || \
...
ssh -o StrictHostKeyChecking=no -i ~/.ssh/${stage_template_vm_ssh_key} ${stage_template_vm_user}@${stage_template_vm_ip} -p ${stage_template_vm_port} \
	"sudo sed -i s/192.168.10.98/${vm_ip}/ /etc/network/interfaces && \
