...
vm_path=$(ssh -o StrictHostKeyChecking=no $kvm_user@$kvm_ip "sudo find /var/lib/libvirt/images -type f -name ${vm_name}*")
...
# Resize procedure:
ssh $kvm_user@$kvm_ip "sudo mv ${vm_path} ${vm_path}.old &&  \
