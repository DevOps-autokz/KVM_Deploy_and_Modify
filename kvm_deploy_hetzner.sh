...
. ${script_path}/.env_hetzner # source env vars for:
...
log_file="${script_path}"/log_file_hetzner.log
...
if [[ $vm_cpu -gt 1 ]]
	...
	virsh $kvm_remote dumpxml $vm_name > $vm_name.xml
         sed -i "/cpu mode/a\    <topology sockets=\'1\' dies=\'1\' cores=\'$vm_cpu\' threads=\'1\'>\n\  </cpu>" $vm_name.xml

#	sed -i "/cpu mode/a\    <topology sockets=\'1\' dies=\'1\' cores=\"$vm_cpu\" threads=\'1\'\/>\n\  </cpu>" $vm_name.xml  
#	sed -i "/cpu mode/a         <topology sockets=\'1\' dies='1' cores='$vm_core' threads='1'\/>\n  </cpu>"   $vm_name.xml
...
function wait_for_SSH { 
	ssh -q -o StrictHostKeyChecking=no -i ~/.ssh/$prod_template_vm_ssh_key $prod_template_vm_user@${prod_template_vm_ip} -p ${prod_template_vm_port} exit || \
...
ssh -o StrictHostKeyChecking=no -i ~/.ssh/${prod_template_vm_ssh_key} ${prod_template_vm_user}@${prod_template_vm_ip} -p ${prod_template_vm_port} \
	"sudo sed -i s/192.168.122.99/${vm_ip}/ /etc/network/interfaces && \
	 sudo sed -i 's/192.168.122.1/192.168.122.1/g' /etc/network/interfaces && \
